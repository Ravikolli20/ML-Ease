...
const App: React.FC = () => {
  ...
  const handleAuthSuccess = (user: UserProfile) => {
    ...
    const projects = mockDb.getProjects(user.id);
  ...
  // Auto-save project (Scoped to User)
  useEffect(() => {
    if (state.user && state.dataset) {
      mockDb.saveProject(state.user.id, {
        id: projectId,
        name: projectName,
      ...
      setHistory(mockDb.getProjects(state.user.id));
    }
  ...
  const updateProfile = (profile: UserProfile) => {
    ...
    // Update master user list in mockDb
  ...
  /**
   * Helper function to render the currently selected pipeline step.
   */
  const renderStep = () => {
  ...
  return (
    <>
      ...
      >
        {/* Fixed Error: Pass the step content as children to Layout component */}
        {renderStep()}
      </Layout>
